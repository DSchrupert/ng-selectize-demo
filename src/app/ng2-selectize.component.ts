...
export class Ng2SelectizeComponent implements OnInit, OnChanges, DoCheck, ControlValueAccessor {
	...
	reset() {
		this.selectize = jQuery(this.selectizeInput.nativeElement).selectize(this.config)[0].selectize;
	...
	evalHasError() {
		if(this.formControl && this.formControl.touched && this.formControl.invalid) {
			jQuery(this.selectize.$control).parent().addClass('has-error');
		} else {
			jQuery(this.selectize.$control).parent().removeClass('has-error');
		}
	}
